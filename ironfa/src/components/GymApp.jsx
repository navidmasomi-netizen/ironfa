...
import { tr, localizedLabel, getInitialLanguage, saveLanguage } from '../utils/translations.js';
import {
  ...
  DEMO_USER, TRUST_BASELINE_COPY, DISCLAIMER_BASELINE_COPY,
  ...
  ACTIVE_PLAN_KEY, WORKOUT_LOG_KEY, ACTIVE_WORKOUT_KEY, PROGRESS_DATA_KEY,
} from '../utils/constants.js';
import {
  canUseStorage, getActivePlan, saveActivePlan, clearActivePlan,
  getPerUserData, savePerUserData, clearPerUserData,
  sanitizeWorkoutLog, sanitizeProgressData,
  normalizePersistedUser, clearSession,
...
function GymApp({ user, onLogout, language = "fa", setLanguage = () => {} }) {
  ...
  const finishWorkout = () => {
    ...
    // Save to backend
    const title = new Date().toLocaleDateString('fa-IR');
    createWorkout(title, user.id, workoutLog.map(e => ({
      name: e.name,
